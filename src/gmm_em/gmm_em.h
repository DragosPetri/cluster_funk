...
namespace cf
{
    ...
    InitializedParameters<nr_components, nr_features> random_init(const Data<nr_features> data)
    {
        ...
        std::vector<double> covariances(nr_components * nr_features * nr_features, 0);
        ...
        std::vector<double> means;
        means.reserve(nr_components * nr_features);

        for (std::uint8_t i = 0; i < nr_components * 1 * nr_features; i++)
        {
    ...
    inline constexpr double norm_factor(std::uint8_t nr_features, double covariance_matrix_det)
    {
    ...
    )
    {
        for (auto component_index : std::views::iota(0uz, responsibilities.extent(1)))
        {
            ...
            auto inv_covariance_data = algebra::gaussJordan<nr_components, nr_features>(covariances, component_index);
    ...
    )
    {
        ...
        for (std::uint8_t component = 0; component < nr_components; component++)
        {
            ...
            auto inv_covariance_data = algebra::gaussJordan<nr_components, nr_features>(covariances, component);
